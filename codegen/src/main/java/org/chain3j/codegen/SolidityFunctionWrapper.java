...
public class SolidityFunctionWrapper extends Generator {
    ...
    private static final String chain3j = "chain3j";
    ...
    private static final String CODEGEN_WARNING = "<p>Auto generated code.\n"
            ...
            + "<p>Please use the "
            + "<a href=\"https://docs.chain3j.io/command_line.html\">chain3j command line tools</a>,\n"
            + "or the " + SolidityFunctionWrapperGenerator.class.getName() + " in the \n"
            + "<a href=\"https://github.com/chain3j/chain3j/tree/master/codegen\">"
    ...
    private void addAddressesSupport(TypeSpec.Builder classBuilder,
        ...
        if (addresses != null) {
            ...
            // See org.chain3j.tx.Contract#getStaticDeployedAddress(String)
    ...
    private String getWeb3jVersion() {
        ...
        try {
            // This only works if run as part of the chain3j command line tools which contains
            // a version.properties file
            version = Version.getVersion();
        } catch (IOException | NullPointerException e) {
            version = Version.DEFAULT;
        }
        return "\n<p>Generated with chain3j version " + version + ".\n";
    }
        ...
        if (withGasProvider) {
            toReturn.addParameter(ContractGasProvider.class, CONTRACT_GAS_PROVIDER)
                    .addStatement("super($N, $N, $N, $N, $N)",
                            BINARY, CONTRACT_ADDRESS, chain3j, authName, CONTRACT_GAS_PROVIDER);
        } else {
            toReturn.addParameter(BigInteger.class, GAS_PRICE)
                    .addParameter(BigInteger.class, GAS_LIMIT)
                    .addStatement("super($N, $N, $N, $N, $N, $N)",
                            BINARY, CONTRACT_ADDRESS, chain3j, authName, GAS_PRICE, GAS_LIMIT)
    ...
    private static MethodSpec buildDeployWithParams(
        ...
        if (isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    ...
                    className, chain3j, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
            methodBuilder.addAnnotation(Deprecated.class);
        } else if (isPayable && withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall("
                            + "$L.class, $L, $L, $L, $L, encodedConstructor, $L)",
                    className, chain3j, authName, CONTRACT_GAS_PROVIDER, BINARY, INITIAL_VALUE);
        } else if (!isPayable && !withGasProvider) {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, $L, encodedConstructor)",
                    className, chain3j, authName, GAS_PRICE, GAS_LIMIT, BINARY);
            methodBuilder.addAnnotation(Deprecated.class);
        } else {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, encodedConstructor)",
                    className, chain3j, authName, CONTRACT_GAS_PROVIDER, BINARY);
        }
        ...
        if (withGasProvider) {
            toReturn.addParameter(ContractGasProvider.class, CONTRACT_GAS_PROVIDER)
                    .addStatement("return new $L($L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, chain3j, authName, CONTRACT_GAS_PROVIDER);
        } else {
            toReturn.addParameter(BigInteger.class, GAS_PRICE)
                    .addParameter(BigInteger.class, GAS_LIMIT)
                    .addStatement("return new $L($L, $L, $L, $L, $L)", className,
                            CONTRACT_ADDRESS, chain3j, authName, GAS_PRICE, GAS_LIMIT)
                    .addAnnotation(Deprecated.class);
        }
    ...
    TypeSpec buildEventResponseObject(
            String className,
            List<org.chain3j.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters,
            List<org.chain3j.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters) {

        TypeSpec.Builder builder = TypeSpec.classBuilder(className)
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC);

        builder.addField(LOG, "log", Modifier.PUBLIC);
        for (org.chain3j.codegen.SolidityFunctionWrapper.NamedTypeName
    ...
    private static Class<?> getStaticArrayTypeReferenceClass(String type) {
        try {
            return Class.forName("org.chain3j.abi.datatypes.generated.StaticArray" + type);
